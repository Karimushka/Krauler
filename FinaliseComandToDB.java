public interface FinaliseComandToDB {
    ...
    String SELECT_REQUEST_FOR_METHOD_ROOTSSITES = "SELECT ID, siteID, url FROM pages WHERE url LIKE '%robots.txt' AND lastScanDate IS NULL;";
    String SELECT_REQUEST_FOR_METHOD_GETIDURL = "SELECT ID, url FROM pages WHERE siteID = ? AND lastScanDate IS NULL;";
    String SELECT_REQUEST_FOR_METHOD_ADD_SITEMAPSANDART = "SELECT ID, siteID, url FROM pages WHERE url LIKE '%sitemap%' AND lastScanDate IS NULL;";
